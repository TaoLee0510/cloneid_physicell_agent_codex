...
def build_selected_trajectory_bundle_record(selection_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "object_types": selected.get("object_types", {}),
        "traversal_policy": selected.get("traversal_policy", {}),
        "root_event_id": selected.get("root_event_id"),
        "rooted_subtree_root_event_ids": selected.get("rooted_subtree_root_event_ids", []),
        "endpoint_event_ids": selected.get("endpoint_event_ids", []),
        "selected_endpoint_event_id": selected.get("selected_endpoint_event_id"),
        "rooted_subtree_event_ids": selected.get("rooted_subtree_event_ids", []),
        "lineage_path_event_ids": selected.get("lineage_path_event_ids", []),
        "candidate_segments_covered": selected.get("candidate_segments_covered", []),
        "connected_candidate_segments": selected.get("connected_candidate_segments", []),
        "passaging_records": selected.get("passaging_records", []),
        "primary_lineage_edges": selected.get("primary_lineage_edges", []),
        "secondary_lineage_edges": selected.get("secondary_lineage_edges", []),
        "lineage_edges": selected.get("lineage_edges", []),
        "segment_connections": selected.get("segment_connections", []),
        "context_transitions_primary": selected.get("context_transitions_primary", []),
        "context_transitions_secondary": selected.get("context_transitions_secondary", []),
        "context_transitions": selected.get("context_transitions", []),
        "perspective_origin_event_ids": selected.get("perspective_origin_event_ids", []),
        "perspective_records": selected.get("perspective_records", []),
        "identity_support_records": selected.get("identity_support_records", []),
...
def write_selected_trajectory_bundle(output_dir: str | Path, selection_payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- Root event: `{selection_payload['selected_record'].get('root_event_id')}`",
        f"- Selected endpoint: `{selection_payload['selected_record'].get('selected_endpoint_event_id')}`",
        ...
        f"- Segment connections: `{len(selection_payload['selected_record'].get('segment_connections', []))}`",
