...
from .modeling_lineage_selection import (
    ...
    write_selected_runtime_lineage_object,
    write_selected_modeling_lineage_object,
    write_runtime_candidate_artifacts,
)
...
def discover_rank_and_select_lineage_objects(
    *,
    ...
    runtime_payload = classify_modeling_candidates_for_runtime(modeling_payload)
    write_runtime_candidate_artifacts(run_dir, runtime_payload)
    runtime_selection = None
    if runtime_payload.get("runtime_eligible_modeling_candidate_count", 0) > 0:
        runtime_selection = select_runtime_lineage_object(runtime_payload)
        write_selected_runtime_lineage_object(run_dir, runtime_selection)
    ...
    summary = {
        ...
        "runtime_eligible_modeling_candidate_count": runtime_payload.get("runtime_eligible_modeling_candidate_count", 0),
        "selected_runtime_lineage_object_id": None if runtime_selection is None else runtime_selection["selected_lineage_object_id"],
        "selected_runtime_lineage_object_type": None if runtime_selection is None else runtime_selection["selected_lineage_object_type"],
    ...
    write_markdown(
        ...
        "\n".join(
            [
                ...
                f"- Runtime-eligible modeling candidate count: `{runtime_payload.get('runtime_eligible_modeling_candidate_count', 0)}`",
                f"- Selected runtime lineage object: `{None if runtime_selection is None else runtime_selection['selected_lineage_object_id']}`",
